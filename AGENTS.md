...
See the **[state-management skill](../skills/state-management/SKILL.md)** for the full state management hierarchy, usehooks-ts hook reference, and code examples for every state type (URL, storage, local, lifted, derived, refs, context, global, form, server).
...
**⚠️ ALL USER-FACING TEXT MUST USE TRANSLATION KEYS** - enforced by ESLint rule `i18next/no-literal-string`.

See the **[i18n skill](../skills/i18n/SKILL.md)** for translation key naming conventions, dynamic content interpolation, form validation with i18n, testing patterns, and locale detection configuration.
