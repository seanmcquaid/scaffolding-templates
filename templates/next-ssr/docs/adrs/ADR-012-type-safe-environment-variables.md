...
For a Next.js SSR application, we also need to:
...
**Description**: Use T3 Env library which wraps Zod with additional Next.js-specific features.

**Pros:**
- Built specifically for Next.js
- Additional safety features
- Automatic NEXT_PUBLIC_ prefix enforcement

**Cons:**
- Additional abstraction layer
- Extra dependency on top of Zod
- More opinionated than plain Zod
- Overkill for straightforward use cases
...
### Server Environment Variables (env.server.ts)
...
### Client Environment Variables (env.client.ts)
...
```typescript
// Server-side code (API routes, server components, getServerSideProps)
import serverEnv from '@/env.server';

console.log(serverEnv.EXAMPLE_SECRET_KEY); // string (typed!)
...
- ⚠️ **Build-Time Variables**: Some Next.js variables must be available at build time
...
- [T3 Stack Environment Variables](https://create.t3.gg/en/usage/env-variables)
