...
# React Native Expo Router Template Setup Script
# This script automates the initial setup process for the React Native Expo Router template
#
...
# Configuration
readonly TEMPLATE_NAME="React Native Expo Router Template"
...
setup_environment() {
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        if [[ "$line" =~ ^[^#]*= ]]; then
            ...
            elif [[ "$var_value" =~ ^(example|dev|test)$ ]] || [[ "$var_value" == "false" && "$var_name" == "EXPO_PUBLIC_DEBUG_MODE" ]]; then
...
show_completion_message() {
    ...
    echo "   pnpm start                   - Start Expo development server"
    echo "   pnpm dev                     - Start with development client"
    echo "   pnpm android                 - Run on Android device/emulator"
    echo "   pnpm ios                     - Run on iOS device/simulator"
    echo "   pnpm web                     - Run on web browser"
    ...
    echo "   pnpm lint:fix                - Run linting with auto-fix"
    echo "   pnpm typecheck               - Run TypeScript type checking"
    echo
    echo "📱 To start development:"
    echo "   1. Install the Expo Go app on your mobile device"
    echo "   2. Run 'pnpm start' to start the development server"
    echo "   3. Scan the QR code with Expo Go (Android) or Camera app (iOS)"
    echo
    echo "Happy coding! 🎯"
}
